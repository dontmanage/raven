...
def on_update(doc, method):
	...
	if doc.has_value_changed("department"):
		...
		if old_doc and old_doc.department:
			...
			if old_channel:
				dontmanage.db.delete("Raven Channel Member", {"channel_id": old_channel, "user_id": raven_user_id})
...
def get_raven_user_for_user(user_id):
	...
	raven_user = dontmanage.get_all("Raven User", {"user": user_id}, pluck="name", limit=1)
