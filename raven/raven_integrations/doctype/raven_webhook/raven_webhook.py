...
class RavenWebhook(Document):
	...
	def before_insert(self):
		...
		webhook = dontmanage.get_all("Raven Webhook", filters={"name": self.name})
		if webhook:
			dontmanage.throw(_("Webhook name already exists"))
	...
	def validate(self):
		...
		if len(webhook_data_keys) != len(set(webhook_data_keys)):
			dontmanage.throw(_("Webhook Data keys should be unique"))
		if len(webhook_header_keys) != len(set(webhook_header_keys)):
			dontmanage.throw(_("Webhook Headers keys should be unique"))
	...
	def on_trash(self):
		# Delete the webhook
		if self.webhook:
			dontmanage.db.delete("Webhook", self.webhook)
	...
	def create_webhook(self):
		...
		webhook_doc = dontmanage.new_doc("Webhook")
	...
	def update_webhook(self):
		...
		conditions = self.get_conditions()
		webhook_doc = dontmanage.get_doc("Webhook", self.webhook)
		webhook_doc.request_url = self.request_url
		webhook_doc.is_dynamic_url = self.is_dynamic_url
		webhook_doc.timeout = self.timeout
		webhook_doc.enable_security = self.enable_security
		...
		if self.trigger_webhook_on_condition:
			if self.conditions_on == "Channel":
				if doctype == "Raven Channel":
					...
					dontmanage.throw(_("Message Reaction cannot be triggered on Channel"))
				elif doctype == "Raven User":
					dontmanage.throw(_("Raven User cannot be triggered on Channel"))

			elif self.conditions_on == "User":
				if doctype == "Raven Channel":
					dontmanage.throw(_("Channel cannot be triggered on User"))
				elif doctype == "Raven Channel Member":
				...
				if doctype == "Raven Channel":
					if self.channel_type in ["Public", "Private", "Open"]:
						...
						dontmanage.throw(_("Invalid Channel Type"))
				else:
					dontmanage.throw(_("Channel Type cannot be triggered on other doctypes"))
