...
from dontmanage.utils import get_datetime, get_system_timezone
...
class RavenMessage(Document):
	...
	def validate_linked_message(self):
		...
		if self.linked_message:
			if (
				dontmanage.get_cached_value("Raven Message", self.linked_message, "channel_id") != self.channel_id
			):
				dontmanage.throw(_("Linked message should be in the same channel"))
	...
	def validate_poll_id(self):
		...
		if self.message_type == "Poll" and not self.poll_id:
			dontmanage.throw(_("Poll ID is mandatory for a poll message"))
	...
	def before_insert(self):
		...
		if self.is_reply and self.linked_message:
			details = dontmanage.db.get_value(
				"Raven Message",
				self.linked_message,
	...
	def handle_ai_message(self):
		...
		raven_settings = dontmanage.get_cached_doc("Raven Settings")
		if not raven_settings.enable_ai_integration:
			return

		# Check if this channel is an AI Thread channel

		channel_doc = dontmanage.get_cached_doc("Raven Channel", self.channel_id)
		...
		peer_user_doc = dontmanage.get_cached_doc("Raven User", peer_user.get("user_id"))

		if peer_user_doc.type != "Bot" or not peer_user_doc.bot:
			return

		bot = dontmanage.get_cached_doc("Raven Bot", peer_user_doc.bot)
	...
	def set_last_message_timestamp(self):
		...
		raven_channel = dontmanage.qb.DocType("Raven Channel")
		query = (
			dontmanage.qb.update(raven_channel)
	...
	def send_push_notification(self):
		...
		if channel_doc.is_direct_message:
			...
			# channel_type = dontmanage.get_cached_value("Raven Channel", self.channel_id, "channel_type")
	...
	def get_message_owner_name(self):
		...
		"""
		if self.is_bot_message:
			return dontmanage.get_cached_value("Raven User", self.bot, "full_name")
		else:
			return dontmanage.get_cached_value("Raven User", self.owner, "full_name")
	...
	def send_notification_for_direct_message(self):
		...
		"""
		peer_raven_user = dontmanage.db.get_value(
		...
		peer_raven_user_doc = dontmanage.get_cached_doc("Raven User", peer_raven_user)
	...
	def send_notification_for_channel_message(self):
		...
		is_thread = dontmanage.get_cached_value("Raven Channel", self.channel_id, "is_thread")
		...
		if is_thread:
			...
			channel_name = dontmanage.get_cached_value("Raven Channel", self.channel_id, "channel_name")
	...
	def send_notification_for_mentions(self, user):
		try:
			from dontmanage.push_notification import PushNotification
			...
			if push_notification.is_enabled():
				push_notification.send_notification_to_user(
					...
					"You were mentioned",
					self.content
					# icon=f"{dontmanage.utils.get_url()}/assets/hrms/manifest/favicon-196.png",
				)
		except ImportError:
			# push notifications are not supported in the current framework version
			pass
		except Exception:
			dontmanage.log_error(dontmanage.get_traceback())
	...
	def after_delete(self):
		dontmanage.publish_realtime(
			...
			# Adding this to automatically add the room for the event via DontManage
			docname=self.channel_id,
		)
		...
		if self.message_type == "Poll":
			dontmanage.delete_doc("Raven Poll", self.poll_id)
	...
	def on_update(self):
		...
		if self.is_edited or self.is_thread or self.flags.editing_metadata:
			...
			if self.message_type != "System":
				...
				# dontmanage.enqueue(method=track_channel_visit, channel_id=self.channel_id, user=self.owner)
	...
	def on_trash(self):
		...
		dontmanage.db.delete("Raven Message Reaction", {"message": self.name})
		...
		if self.is_thread:
			...
			thread_channel_doc = dontmanage.get_doc("Raven Channel", self.name)
...
def on_doctype_update():
	...
	dontmanage.db.add_index("Raven Message", ["channel_id", "creation"])
	dontmanage.db.add_index("Raven Message", ["message_type", "creation"])
