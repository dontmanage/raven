...
def get_search_result(
	...
	file_doc = dontmanage.qb.DocType("File")
	...
	if my_channel_only == "true":
		query = query.where((channel.type == "Open") | (channel_member.user_id == dontmanage.session.user))

	if saved == "true":
		query = query.where(message._liked_by.like(f"%{dontmanage.session.user}%"))
