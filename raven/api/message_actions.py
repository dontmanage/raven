...
def get_action_defaults(action_id: str, message_id: str):
	...
	dontmanage.has_permission(doctype="Raven Message", doc=message_id, ptype="read", throw=True)
	action = dontmanage.get_doc("Raven Message Action", action_id)
	message = dontmanage.get_doc("Raven Message", message_id)
	...
	for field in action.fields:
		...
		if field.default_value_type == "Jinja":
			val = dontmanage.render_template(field.default_value, {"message": message})
...
def execute_action(action_id: str, message_id: str, values: dict):
	...
	if action.action == "Create Document":
		doc = dontmanage.get_doc({"doctype": action.document_type, **values})
		doc.insert()
	...
	if action.action == "Custom Function":
		...
		function_name = dontmanage.get_attr(action.custom_function_path)

		if function_name:
			return function_name(**values)
		else:
			dontmanage.throw(_("Function {0} not found").format(action.custom_function_path))
