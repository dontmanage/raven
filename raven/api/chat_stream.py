...
def get_messages(channel_id: str, limit: int = 20, base_message: str | None = None):
	...
	if len(messages) == limit:
		...
		older_message = dontmanage.db.get_all(
			"Raven Message",
			pluck="name",
...
def get_messages_around_base(channel_id: str, base_message: str):
	...
	from_timestamp = dontmanage.get_cached_value("Raven Message", base_message, "creation")
...
def get_older_messages(channel_id: str, from_message: str, limit: int = 20):
	...
	if not dontmanage.has_permission(doctype="Raven Channel", doc=channel_id, ptype="read"):
		dontmanage.throw(_("You do not have permission to access this channel"), dontmanage.PermissionError)
	# Fetch older messages for the channel
	from_timestamp = dontmanage.get_cached_value("Raven Message", from_message, "creation")
...
def fetch_newer_messages(
	...
	if len(messages) == limit:
		# Check if there are more messages available
		newer_message = dontmanage.db.get_all(
			"Raven Message",
			pluck="name",
			filters={
				"channel_id": channel_id,
