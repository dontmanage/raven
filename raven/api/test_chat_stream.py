...
def create_channel():
	channel_doc = dontmanage.get_doc(
		{
...
class TestChatStream(IntegrationTestCase):
	def setUp(self):
		try:
			dontmanage.delete_doc("Raven Channel", CHANNEL_ID)
		except dontmanage.exceptions.DoesNotExistError:
			pass
		dontmanage.clear_cache(doctype="Raven Message")
		create_channel()
	...
	def test_get_messages_around_base_mid(self):
		...
		"""
		base_message_id = dontmanage.db.get_value(
