...
def create_poll(
	...
	if not dontmanage.has_permission(doctype="Raven Channel", doc=channel_id, ptype="read"):
		dontmanage.throw(_("You do not have permission to access this channel"), dontmanage.PermissionError)

	poll = dontmanage.get_doc(
		{
			"doctype": "Raven Poll",
			"question": question,
...
def get_poll(message_id):
	...
	if not dontmanage.has_permission(doctype="Raven Message", doc=message_id, ptype="read"):
		dontmanage.throw(_("You do not have permission to access this message"), dontmanage.PermissionError)

	poll_id = dontmanage.get_cached_value("Raven Message", message_id, "poll_id")
	...
	current_user_vote = dontmanage.get_all(
		"Raven Poll Vote",
		filters={"poll_id": poll_id, "user_id": dontmanage.session.user},
		fields=["option"],
	)
...
def add_vote(message_id, option_id):
	...
	poll_id = dontmanage.get_cached_value("Raven Message", message_id, "poll_id")
	is_poll_multi_choice = dontmanage.get_cached_value("Raven Poll", poll_id, "is_multi_choice")
...
def retract_vote(poll_id):
	...
	votes = dontmanage.get_all(
		"Raven Poll Vote", filters={"poll_id": poll_id, "user_id": user}, fields=["name"]
	)
	if not votes:
		dontmanage.throw(_("You have not voted for any option in this poll."))
	else:
		for vote in votes:
			dontmanage.delete_doc("Raven Poll Vote", vote.name)
...
def get_all_votes(poll_id):
	...
	if not dontmanage.has_permission(doctype="Raven Poll", doc=poll_id, ptype="read"):
		dontmanage.throw(_("You do not have permission to access this poll"), dontmanage.PermissionError)

	poll_doc = dontmanage.get_cached_doc("Raven Poll", poll_id)
