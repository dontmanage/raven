...
def create_thread(message_id):
	...
	thread_message = dontmanage.get_doc("Raven Message", message_id)

	channel_doc = dontmanage.get_cached_doc("Raven Channel", thread_message.channel_id)

	# Convert the message to a thread
	thread_channel = dontmanage.get_doc(
		{
	...
	if creator != dontmanage.session.user:
		dontmanage.get_doc(
