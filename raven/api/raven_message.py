...
from dontmanage.query_builder import Case, JoinType, Order
from dontmanage.query_builder.functions import Coalesce, Count
...
def fetch_recent_files(channel_id):
	...
	"""
	if not dontmanage.has_permission("Raven Channel", doc=channel_id):
		dontmanage.throw(_("You don't have permission to view this channel"), dontmanage.PermissionError)
	files = dontmanage.db.get_all(
		"Raven Message",
...
def get_messages(channel_id):
	...
	messages = dontmanage.db.get_all(
		"Raven Message",
...
def save_message(message_id, add=False):
	...
	"""
	from dontmanage.desk.like import toggle_like

	toggle_like("Raven Message", message_id, add)

	liked_by = dontmanage.db.get_value("Raven Message", message_id, "_liked_by")
...
def get_saved_messages():
	...
	query = (
		...
		.where(raven_message._liked_by.like("%" + dontmanage.session.user + "%"))
		.where(
			(raven_channel.type.isin(["Open", "Public"]))
			| (raven_channel_member.user_id == dontmanage.session.user)
		)
...
def check_permission(channel_id):
	if dontmanage.get_cached_value("Raven Channel", channel_id, "type") == "Private":
...
def get_unread_count_for_channels():
	...
	message = dontmanage.qb.DocType("Raven Message")
	query = (
		dontmanage.qb.from_(channel)
		.left_join(channel_member)
		.on(
			(channel.name == channel_member.channel_id) & (channel_member.user_id == dontmanage.session.user)
		)
		.where(channel_member.user_id == dontmanage.session.user)
...
def get_unread_count_for_channel(channel_id):
	...
	if channel_member:
		last_timestamp = dontmanage.get_cached_value(
		...
		if dontmanage.get_cached_value("Raven Channel", channel_id, "type") == "Open":
			return dontmanage.db.count(
				"Raven Message",
				filters={
					"channel_id": channel_id,
...
def get_timeline_message_content(doctype, docname):
	...
	for log in data:
		...
		if log.is_direct_message:
			...
			if peer_user_id:
				log["peer_user"] = dontmanage.db.get_value("User", peer_user_id, "full_name")
...
def get_all_files_shared_in_channel(
	...
	message = dontmanage.qb.DocType("Raven Message")
	user = dontmanage.qb.DocType("Raven User")
	file = dontmanage.qb.DocType("File")
	...
	doc = dontmanage.get_doc(
		{
			...
			**forwarded_message,
			...
			"creation": dontmanage.utils.now_datetime(),
			"modified": dontmanage.utils.now_datetime(),
			"is_continuation": 0,
			"is_edited": 0,
			"is_reply": 0,
			"is_forwarded": 1,
