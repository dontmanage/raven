...
from dontmanage.query_builder import JoinType, Order
...
def get_list():
	...
	"""
	if not dontmanage.db.exists("Raven User", {"user": dontmanage.session.user}):
		dontmanage.throw(_("You do not have access to Raven."), dontmanage.PermissionError)

	workspace = dontmanage.qb.DocType("Raven Workspace")
	...
	all_workspaces = (
		...
		.on(
			(workspace.name == workspace_member.workspace) & (workspace_member.user == dontmanage.session.user)
		)
		.where((workspace_member.user == dontmanage.session.user) | (workspace.type == "Public"))
		.select(
...
def join_workspace(workspace: str):
	...
	member = dontmanage.get_doc(
		{"doctype": "Raven Workspace Member", "workspace": workspace, "user": dontmanage.session.user}
	).insert()
...
def leave_workspace(workspace: str):
	...
	Leaves a workspace
	"""
	member = dontmanage.db.exists(
		"Raven Workspace Member", {"workspace": workspace, "user": dontmanage.session.user}
	)
	if member:
		member_doc = dontmanage.get_doc("Raven Workspace Member", member)
		member_doc.delete()

	else:
		dontmanage.throw(_("You are not a member of this workspace."), dontmanage.PermissionError)
...
def is_workspace_admin(workspace: str):
	...
	"""
	workspace_member = dontmanage.get_doc(
		"Raven Workspace Member", {"workspace": workspace, "user": dontmanage.session.user}
	)
...
def fetch_workspace_members(workspace: str):
	...
	"""
	dontmanage.has_permission("Raven Workspace", doc=workspace, throw=True)
...
def add_workspace_members(workspace: str, members: list):
	...
	"""
	dontmanage.has_permission("Raven Workspace", doc=workspace, ptype="write", throw=True)
...
def update_workspace_members(workspace: str, members: list):
	...
	if not is_workspace_admin(workspace):
		dontmanage.throw(_("You are not an admin of this workspace."), dontmanage.PermissionError)
	...
	for member in members:
		try:
			...
			member_id = dontmanage.db.exists("Raven Workspace Member", {"workspace": workspace, "user": user})

			# If not a member, delete the member
			if not is_member:
				if member_id:
					member_doc = dontmanage.get_doc("Raven Workspace Member", member_id)
					member_doc.flags.ignore_cache_invalidation = True
	...
	delete_workspace_members_cache(workspace)
	workspace_channels = dontmanage.get_all("Raven Channel", {"workspace": workspace}, pluck="name")
