...
def get_document(doctype: str, document_id: str):
	...
	"""
	# Use the dontmanage.client.get method to get the document with permissions (both read and field level read)
	return dontmanage.client.get(doctype, name=document_id)
...
def get_documents(doctype: str, document_ids: list):
	...
	for document_id in document_ids:
		# Use the dontmanage.client.get method to get the document with permissions applied
		docs.append(dontmanage.client.get(doctype, name=document_id))
	return docs
...
def create_document(doctype: str, data: dict, function=None):
	...
	doc = dontmanage.get_doc({"doctype": doctype, **data})
	doc.insert()
...
def update_document(doctype: str, document_id: str, data: dict, function=None):
	...
	doc = dontmanage.get_doc(doctype, document_id)
	doc.update(data)
	doc.save()
...
def delete_document(doctype: str, document_id: str):
	...
	"""
	dontmanage.delete_doc(doctype, document_id)
...
def attach_file_to_document(doctype: str, document_id: str, file_path: str):
	...
	"""
	if not dontmanage.db.exists(doctype, document_id):
		return {
	...
	file = dontmanage.get_doc("File", {"file_url": file_path})

	if not file:
		dontmanage.throw(_("File not found"))

	newFile = dontmanage.get_doc(
		{
			"doctype": "File",
			"file_url": file_path,
...
def get_list(doctype: str, filters: dict = None, fields: list = None, limit: int = 20):
	...
	# Use the dontmanage.get_list method to get the list of documents
	return dontmanage.get_list(doctype, filters=filters, fields=fields, limit=limit)
