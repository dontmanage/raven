	...
	class EventHandler(AssistantEventHandler):
		...
		def handle_requires_action(self, data, run_id):
			...
			for tool in data.required_action.submit_tool_outputs.tool_calls:
				...
				try:
					function = dontmanage.get_cached_doc("Raven AI Function", tool.function.name)
				...
				try:
					...
					if function.type == "Custom Function":
						function_name = dontmanage.get_attr(function.function_path)
						...
						if bot.allow_bot_to_write_documents:
							...
							dontmanage.db.savepoint(run_id + "_" + tool.id)
							if function.pass_parameters_as_json:
								function_output = function_name(args)
							else:
								function_output = function_name(**args)
							dontmanage.db.rollback(save_point=run_id + "_" + tool.id)
					...
					dontmanage.log_error("Raven AI Error", dontmanage.get_traceback())

					if bot.debug_mode:
						bot.send_message(
							channel_id=channel_id,
							text=f"<details data-summary='Error in function call'><p>{dontmanage.get_traceback()}</p></details>",
						)
					tool_outputs.append(
						{
...
def get_instructions(bot):
	...
	instructions = dontmanage.render_template(bot.instruction, vars)
	return instructions


def get_variables_for_instructions():
	user = dontmanage.get_cached_doc("User", dontmanage.session.user)
	...
	if "dontmanageerp" in dontmanage.get_installed_apps():
		employee_id = dontmanage.db.exists("Employee", {"user_id": dontmanage.session.user})

		if employee_id:
			employee = dontmanage.get_cached_doc("Employee", employee_id)
			employee_company = employee.company
			department = employee.department

		company = dontmanage.defaults.get_user_default("company") or dontmanage.db.get_single_value(
