...
class TestPermissions(IntegrationTestCase):
	def setUp(self):
		...
		user1 = dontmanage.get_doc("User", "test1@example.com")
		user1.add_roles("Raven User")
		user1.remove_roles("Raven Admin")

		user2 = dontmanage.get_doc("User", "test2@example.com")
		user2.remove_roles("Raven Admin")
		user2.remove_roles("Raven User")

		user3 = dontmanage.get_doc("User", "test3@example.com")
	...
	def create_test_workspace(self, workspace_type="Public"):
		test_workspace = dontmanage.get_doc(
	...
	def create_test_channel(
		...
		test_channel = dontmanage.get_doc(
			{
	...
	def create_test_message(self, channel_id):
		test_message = dontmanage.get_doc(
			{
	...
	def test_workspace_update_permissions(self):
		...
		test_workspace_member = dontmanage.get_doc(
			{
	...
	def test_workspace_read_permissions(self):
		...
		self.assertTrue(dontmanage.has_permission(test_workspace, ptype="read"))

		# test2 should not be able to read the workspace
		dontmanage.set_user("test2@example.com")
		self.assertFalse(dontmanage.has_permission(test_workspace, ptype="read"))
	...
	def test_workspace_member_create_permissions(self):
		...
		test1_workspace_member = dontmanage.get_doc(
			{
				"doctype": "Raven Workspace Member",
				"user": "test1@example.com",
				"workspace": test_workspace.name,
		...
		test3_workspace_member = dontmanage.get_doc(
			{
				"doctype": "Raven Workspace Member",
		...
		self.assertEqual(
			2, dontmanage.db.count("Raven Workspace Member", {"workspace": test_workspace.name})
		)
		...
		self.assertEqual(
			3, dontmanage.db.count("Raven Workspace Member", {"workspace": test_workspace.name})
		)
	...
	def test_workspace_member_delete_permissions(self):
		...
		self.assertEqual(
			1, dontmanage.db.count("Raven Workspace Member", {"workspace": test_workspace.name})
		)
	...
	def test_workspace_member_read_permissions(self):
		...
		dontmanage.set_user("test1@example.com")
		self.assertTrue(dontmanage.has_permission(test3_workspace_member, ptype="read"))
		...
		dontmanage.set_user("test3@example.com")
		self.assertFalse(dontmanage.has_permission(test1_workspace_member, ptype="read"))
	...
	def test_channel_create_permissions(self):
		...
		test1_channel_member = dontmanage.get_doc(
			{
		...
		self.assertEqual(1, dontmanage.db.count("Raven Channel Member", {"channel_id": test_channel.name}))
	...
	def test_channel_delete_permissions(self):
		...
		self.assertFalse(dontmanage.db.exists("Raven Channel", test_channel.name))
		...
		dontmanage.set_user("test@example.com")
		with self.assertRaises(dontmanage.PermissionError):
			dontmanage.delete_doc("Raven Channel", dm_t1_t3)
		...
		with self.assertRaises(dontmanage.PermissionError):
			dontmanage.delete_doc("Raven Channel", test_message.name)

		# Delete the message
		test_message.delete()

		# The thread channel should be deleted automatically
		self.assertFalse(dontmanage.db.exists("Raven Channel", test_message.name))
	...
	def test_channel_read_permissions(self):
		...
		self.assertTrue(dontmanage.has_permission("Raven Channel", doc=public_channel.name))
		self.assertTrue(dontmanage.has_permission("Raven Channel", doc=open_channel.name))
		...
		dontmanage.set_user("test3@example.com")
		self.assertFalse(dontmanage.has_permission("Raven Channel", doc=public_channel.name))
		self.assertFalse(dontmanage.has_permission("Raven Channel", doc=open_channel.name))

		# Test should be able to read the private channel, test1 and test3 should not be able to read the private channel
		dontmanage.set_user("test@example.com")
		self.assertTrue(dontmanage.has_permission("Raven Channel", doc=private_channel.name))

		dontmanage.set_user("test1@example.com")
		self.assertFalse(dontmanage.has_permission("Raven Channel", doc=private_channel.name))
		...
		self.assertTrue(dontmanage.has_permission("Raven Channel", doc=test_message.name))

		# Test1 should not be able to read the thread since they are not a member of the channel
		dontmanage.set_user("test1@example.com")
		self.assertFalse(dontmanage.has_permission("Raven Channel", doc=test_message.name))
	...
	def test_channel_member_permissions(self):
		...
		test1_open_channel_member = dontmanage.get_doc(
			{
				"doctype": "Raven Channel Member",
		...
		test1_public_channel_member = dontmanage.get_doc(
			{
				"doctype": "Raven Channel Member",
		...
		dontmanage.set_user("test@example.com")
		test1_private_channel_member = dontmanage.get_doc(
			{
				"doctype": "Raven Channel Member",
				"channel_id": private_channel.name,
				"user_id": "test1@example.com",
		...
		self.assertTrue(
			dontmanage.has_permission("Raven Channel Member", doc=test1_open_channel_member.name, ptype="write")
		)
		self.assertTrue(
		...
		self.assertTrue(
			dontmanage.db.get_value("Raven Channel Member", test1_open_channel_member.name, "is_admin")
		)
		self.assertTrue(
			dontmanage.db.get_value("Raven Channel Member", test1_private_channel_member.name, "is_admin")
		)
		...
		test3_open_channel_member = dontmanage.get_doc(
			{
				"doctype": "Raven Channel Member",
				"channel_id": open_channel.name,
				"user_id": "test3@example.com",
		...
		test3_private_channel_member = dontmanage.get_doc(
			{
				"doctype": "Raven Channel Member",
				"channel_id": private_channel.name,
				"user_id": "test3@example.com",
		...
		dontmanage.set_user("test1@example.com")
		self.assertTrue(dontmanage.has_permission("Raven Channel", doc=thread["thread_id"], ptype="read"))

		# Test1 can join the thread channel
		test1_thread_channel_member = dontmanage.get_doc(
			{
				"doctype": "Raven Channel Member",
	...
	def test_message_permissions(self):
		...
		self.assertTrue(dontmanage.has_permission("Raven Message", doc=test_message.name, ptype="read"))
		...
		self.assertFalse(dontmanage.db.exists("Raven Message", test_message.name))
		...
		self.assertFalse(dontmanage.db.exists("Raven Message", test1_message.name))
