...
def check_app_permission():
	...
	if dontmanage.db.exists("Raven User", {"user": dontmanage.session.user}):
		return True
...
def workspace_member_has_permission(doc, user=None, ptype=None):
	...
	if ptype == "create":
		workspace_type = dontmanage.get_cached_value("Raven Workspace", doc.workspace, "type")
...
def channel_has_permission(doc, user=None, ptype=None):
	if doc.is_direct_message:
		...
		if ptype == "create":
			...
			main_channel = dontmanage.db.get_value("Raven Message", doc.channel_name, "channel_id")
	...
	# 	if doc.workspace:
	# 		return dontmanage.db.exists("Raven Workspace Member", {"workspace": doc.workspace, "user": user})
	# 	return True
	# elif doc.type == "Private":
	# 	if doc.is_thread:
	# 		if ptype == "read" or ptype == "create":
	# 			# Only users who are part of the original channel can read the thread
	# 			return dontmanage.has_permission(doctype="Raven Message", doc=doc.name, ptype="read")

	# 	if dontmanage.db.exists("Raven Channel Member", {"channel_id": doc.name, "user_id": user}):
	# 		return True
	# 	elif (
	# 		doc.owner == user and dontmanage.db.count("Raven Channel Member", {"channel_id": doc.name}) <= 0
	# 	):
	# 		return True
	# 	else:
	# 		return False
...
def channel_member_has_permission(doc, user=None, ptype=None):
	...
	if ptype == "read":
		...
		return dontmanage.db.exists("Raven Channel Member", {"channel_id": doc.channel_id, "user_id": user})

	channel_doc = dontmanage.get_cached_doc("Raven Channel", doc.channel_id)
	...
	if ptype == "create":
		if is_direct_message:
			...
			if channel_doc.is_thread:
				# Check if the user is a member of the main channel
				main_channel = dontmanage.db.get_value("Raven Message", doc.channel_id, "channel_id")
				return is_channel_member(main_channel, user)
	...
	# 	if dontmanage.db.exists("Raven Channel Member", {"channel_id": doc.channel_id, "user_id": user}):
	# 		return True
...
def raven_poll_vote_has_permission(doc, user=None, ptype=None):
	...
	if not dontmanage.has_permission("Raven Poll", doc=doc.poll_id, ptype="read", user=user):
		return False
	...
	if ptype in ["read", "create", "delete"]:
		if doc.owner == user:
			...
			is_anonymous = dontmanage.get_cached_value("Raven Poll", doc.poll_id, "is_anonymous")
			if not is_anonymous:
				if ptype == "read":
					return True
			else:
...
def raven_poll_has_permission(doc, user=None, ptype=None):
	...
	if ptype == "read" or ptype == "delete" or ptype == "write":
		...
		message = dontmanage.get_doc("Raven Message", {"poll_id": doc.name})
...
def raven_channel_query(user):
    ...
    """
	return f"`tabRaven Channel`.owner = {dontmanage.db.escape(user)}"
...
def raven_message_query(user):
    ...
    """
	return f"`tabRaven Message`.owner = {dontmanage.db.escape(user)}"
...
def raven_poll_query(user):
	...
	return f"`tabRaven Poll`.owner = {dontmanage.db.escape(user)}"
...
def raven_poll_vote_query(user):
	...
	"""
	return f"`tabRaven Poll Vote`.owner = {dontmanage.db.escape(user)}"
