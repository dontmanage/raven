	...
	def validate(self):
		if self.auto_create_department_channel:
			if not self.company_workspace_mapping:
				dontmanage.throw(_("Please map the companies to the workspace before enabling this feature."))

			for row in self.company_workspace_mapping:
				# Check if the company exists since it's a Data field
				if not dontmanage.db.exists("Company", row.company):
					dontmanage.throw(f"Company {row.company} does not exist.")
