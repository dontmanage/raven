...
class RavenUser(Document):
	...
	def validate(self):
		if self.type == "Bot" and not self.bot:
			dontmanage.throw(_("Bot is mandatory"))

		if self.type == "User" and not self.user:
			dontmanage.throw(_("User is mandatory"))
	...
	def on_trash(self):
		...
		"""
		dontmanage.db.delete("Raven Channel Member", {"user_id": self.user})
	...
	def update_photo_from_user(self):
		...
		and DontManage creates a duplicate file in the system (that is public) but does not update the URL in the field.
		"""
		user_image = dontmanage.db.get_value("User", self.user, "user_image")
		if user_image and not self.user_image:
			image_file = dontmanage.get_doc(
				{
					"doctype": "File",
					"file_url": user_image,
...
def add_user_to_raven(doc, method):
	...
	if not doc.flags.deleting_raven_user:
		if dontmanage.db.exists("Raven User", {"user": doc.name}):
			...
			if has_raven_role:
				raven_user = dontmanage.get_doc("Raven User", {"user": doc.name})
			...
			if dontmanage.db.exists("User", doc.name):
				# Check if the user is a system user.
				auto_add = False
				if doc.user_type == "System User":
					auto_add = dontmanage.db.get_single_value("Raven Settings", "auto_add_system_users")
