...
class RavenWorkspaceMember(Document):
	...
	def validate(self):
		...
		if dontmanage.db.exists("Raven Workspace Member", conditions):
			dontmanage.throw(_("User is already a member of the workspace"))
	...
	def validate_other_admins_exist(self):
		...
		if self.has_value_changed("is_admin") and not self.is_admin:
			other_admins = dontmanage.db.count(
	...
	def check_last_admin(self):
		...
		"""
		other_admin = dontmanage.db.count(
			"Raven Workspace Member",
			{"workspace": self.workspace, "is_admin": True, "name": ["!=", self.name]},
		)
		if other_admin == 0:
	...
	def delete_channel_members_for_user(self):
		...
		"""
		workspace_channels = dontmanage.get_all("Raven Channel", {"workspace": self.workspace}, pluck="name")
		for channel in workspace_channels:
			dontmanage.db.delete("Raven Channel Member", {"channel_id": channel, "user_id": self.user})
...
def on_doctype_update():
	...
	dontmanage.db.add_index("Raven Workspace Member", ["workspace", "user"])
