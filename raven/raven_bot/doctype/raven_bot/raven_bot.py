...
class RavenBot(Document):
	...
	def validate(self):
		if self.is_ai_bot and not self.instruction:
			dontmanage.throw(_("Please provide an instruction for this AI Bot."))
	...
	def validate_functions(self):
		if not self.allow_bot_to_write_documents:
			for f in self.bot_functions:
				needs_write = dontmanage.db.get_value(
	...
	def on_update(self):
		...
		"""
		if self.raven_user:
			raven_user = dontmanage.get_doc("Raven User", self.raven_user)
			raven_user.type = "Bot"
			...
			raven_user = dontmanage.new_doc("Raven User")
			raven_user.type = "Bot"
			raven_user.bot = self.name
			raven_user.full_name = self.bot_name
			raven_user.first_name = self.bot_name
	...
	def on_trash(self):
		...
		if self.raven_user:
			dontmanage.db.set_value("Raven User", self.raven_user, "bot", None)
			self.db_set("raven_user", None)
			dontmanage.delete_doc("Raven User", self.raven_user)
	...
	def get_tools_for_assistant(self):
		...
		for f in self.bot_functions:
			function_def = dontmanage.db.get_value("Raven AI Function", f.function, "function_definition")
			if function_def:
	...
	def is_member(self, channel_id: str) -> None | str:
		...
		"""
		member_id = dontmanage.db.exists(
	...
	def add_to_channel(self, channel_id: str) -> str:
		...
		if not existing_member:
			raven_channel_member = dontmanage.get_doc(
	...
	def remove_from_channel(self, channel_id: str) -> None:
		...
		existing_member = self.is_member(channel_id)
		if existing_member:
			dontmanage.delete_doc("Raven Channel Member", existing_member)
	...
	def get_dm_channel_id(self, user_id: str) -> str | None:
		...
		user_raven_user = dontmanage.db.get_value("Raven User", {"user": user_id}, "name")
	...
	def send_message(
		...
		if markdown:
			text = dontmanage.utils.md_to_html(text)
	...
	def create_direct_message_channel(self, user_id: str) -> str:
		...
		if channel_id:
			...
			if not user_raven_user:
				dontmanage.throw(f"User {user_id} is not added as a Raven User")
	...
	def get_last_message(self, channel_id: str = None, message_type: str = None) -> Document | None:
		...
		return dontmanage.get_last_doc("Raven Message", filters=filters, order_by="creation desc")
	...
	def get_previous_messages(
		...
		return dontmanage.get_all("Raven Message", filters=filters, order_by="creation desc", pluck="name")
