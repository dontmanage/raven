...
def execute():
	...
	thread_channel = dontmanage.qb.DocType("Raven Channel")
	message = dontmanage.qb.DocType("Raven Message")
	message_channel = dontmanage.qb.DocType("Raven Channel")

	query = (
		dontmanage.qb.from_(thread_channel)
		.join(message)
	...
	for result in results:
		dontmanage.db.set_value("Raven Channel", result.name, {"is_dm_thread": 1, "workspace": None})
