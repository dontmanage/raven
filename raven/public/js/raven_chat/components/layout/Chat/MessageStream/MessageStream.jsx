...
const MessageStream = ({ channelID }) => {
  ...
  const { data } = useSWRSubscription(
    ...
    (key, { next }) => {
      ...
      if (channelID) {
        dontmanage.socketio.doc_subscribe("Raven Channel", channelID);
        dontmanage.socketio.doc_open("Raven Channel", channelID)
        dontmanage.realtime.on("message_updated", (event) => {
      ...
      return () => {
        dontmanage.realtime.off("message_updated");
        dontmanage.socketio.doc_close("Raven Channel", channelID);
        try {
          dontmanage.socketio.doc_unsubscribe("Raven Channel", channelID);
        } catch (e) {
          // console.log(e)
        }
      }
