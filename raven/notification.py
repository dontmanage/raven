...
def send_notification_to_user(user_id, title, message, data=None, user_image_id=None):
	...
	try:
		...
		if data is None:
			data = {
				"base_url": dontmanage.utils.get_url(),
			}
		else:
			data["base_url"] = dontmanage.utils.get_url()

		if push_notification.is_enabled():
			icon_url = None
			if user_image_id:
				icon = dontmanage.get_cached_value("Raven User", user_image_id, "user_image")
				if icon:
					icon_url = dontmanage.utils.get_url() + icon

			link = None
			if data.get("channel_id"):
				link = dontmanage.utils.get_url() + "/raven/channel/" + data.get("channel_id", "")
		...
		dontmanage.log_error("Failed to send push notification")
...
def subscribe_user_to_topic(channel_id, user_id):
	...
	try:
		...
		dontmanage.log_error("Failed to subscribe user to channel")
...
def unsubscribe_user_to_topic(channel_id, user_id):
	...
	try:
		...
		dontmanage.log_error("Failed to unsubscribe user to channel")
