...
class RavenAIFunction(Document):
	...
	def validate_reference_doctype(self):
		if not self.reference_doctype:
			if not self.type in [
				...
				dontmanage.throw(_("Please select a DocType for this function."))
	...
	def validate_fields_for_doctype(self):
		...
		doctype = dontmanage.get_meta(self.reference_doctype)
		...
		for param in self.parameters:
			...
			if param.child_table_name:
				...
				child_meta = dontmanage.get_meta(child_table.options)
				if not child_meta:
					dontmanage.throw(_("Child table {0} is not a valid doctype").format(param.child_table_name))

				# Check if the child table variable has a valid name
				docfield = child_meta.get_field(param.fieldname)
				if not docfield:
					dontmanage.throw(_("Field {0} not found in {1}").format(param.fieldname, child_table.options))
			else:

				field = doctype.get_field(param.fieldname)

				if not field:
					dontmanage.throw(_("Field {0} not found in {1}").format(param.fieldname, self.reference_doctype))

				if field.fieldtype == "Select":
					if not param.options:
						dontmanage.throw(_("Options are required for select fields"))
	...
	def build_params_json_from_table(self):
		...
		for child_table_name, child_table in child_tables.items():
			doctype_meta = dontmanage.get_meta(self.reference_doctype)
	...
	def validate_json(self):
		if self.type == "Custom Function":
			if not self.function_path:
				dontmanage.throw(_("Function path is required for Custom Functions"))

			# Check if JSON is valid and format it
			try:
				json.loads(self.params)
			except json.JSONDecodeError:
				dontmanage.throw(_("Invalid JSON in params"))
	...
	def validate(self):
		...
		if self.type == "Custom Function":
			f = dontmanage.get_attr(self.function_path)
			if not f:
				dontmanage.throw(_("Function not found"))
	...
	def on_update(self):
		...
		bots = dontmanage.get_all("Raven Bot Functions", filters={"function": self.name}, pluck="parent")

		for bot in bots:
			bot = dontmanage.get_doc("Raven Bot", bot)
