...
// const NO_CACHE_KEYS = [
//   "dontmanage.desk.form.load.getdoctype",
//   "dontmanage.desk.search.search_link",
//   "dontmanage.model.workflow.get_transitions",
//   "dontmanage.desk.reportview.get_count",
//   "dontmanage.core.doctype.server_script.server_script.enabled",
      ...
      <Route path="/" element={<ProtectedRoute />} errorElement={<ErrorPage />}>
        <Route path="/" element={<WorkspaceSwitcher />}>
          ...
          <Route path="settings" lazy={() => import('./pages/settings/Settings')}>
            ...
            <Route path="hr" lazy={() => import('./pages/settings/Integrations/DontManageHR')} />
...
function App() {
  ...
  // We not need to pass sitename if the DontManage version is v14.

  const getSiteName = () => {
    // @ts-ignore
    if (window.dontmanage?.boot?.versions?.dontmanage.startsWith('14')) {
      return import.meta.env.VITE_SITE_NAME
    }
    // @ts-ignore
    else {
      // @ts-ignore
      return window.dontmanage?.boot?.sitename ?? import.meta.env.VITE_SITE_NAME
    }
  }

  return (
    <DontManageProvider
      url={import.meta.env.VITE_DONTMANAGE_PATH ?? ''}
