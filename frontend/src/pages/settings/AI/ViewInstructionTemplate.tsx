...
import { SWRResponse, useDontManageGetDoc, useDontManageUpdateDoc } from "dontmanage-react-sdk"
...
const ViewInstructionTemplate = () => {
    ...
    const { data, isLoading, error, mutate } = useDontManageGetDoc<RavenBotInstructionTemplate>("Raven Bot Instruction Template", ID)
...
const ViewBotContent = ({ data, mutate }: { data: RavenBotInstructionTemplate, mutate: SWRResponse['mutate'] }) => {
    ...
    const { updateDoc, loading, error } = useDontManageUpdateDoc<RavenBotInstructionTemplate>()
