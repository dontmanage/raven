...
const Appearance = () => {
    ...
    const { data: chatStyle, mutate } = useDontManageGetCall<{ message: { chat_style: 'Simple' | 'Left-Right' } }>('dontmanage.client.get_value', {
        doctype: 'Raven User',
    ...
    const { call } = useDontManagePostCall('dontmanage.client.set_value')
    ...
    const setChatStyle = (style: string) => {
        call({
            ...
            // @ts-expect-error
            window.dontmanage.boot.chat_style = style
            mutate()
