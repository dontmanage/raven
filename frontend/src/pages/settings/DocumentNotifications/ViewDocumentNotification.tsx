...
import { useDontManageGetDoc, useDontManageUpdateDoc, SWRResponse } from "frappe-react-sdk"
...
const ViewDocumentNotification = (props: Props) => {
    ...
    const { data, isLoading, error, mutate } = useDontManageGetDoc<RavenDocumentNotification>("Raven Document Notification", ID)
...
const ViewDocumentNotificationContent = ({ data, mutate }: { data: RavenDocumentNotification, mutate: SWRResponse['mutate'] }) => {
    ...
    const { updateDoc, loading, error } = useDontManageUpdateDoc<RavenDocumentNotification>()
