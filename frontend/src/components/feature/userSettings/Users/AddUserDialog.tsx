...
const UserForm = ({ onClose }: { onClose: VoidFunction }) => {
    ...
    const { call } = useContext(DontManageContext) as DontManageConfig

    const { loading, call: inviteUser, error } = useDontManagePostCall('raven.api.raven_users.invite_user')
    ...
    const onEmailBlur = () => {
        ...
        if (user) {
            ...
            setRavenUserExists(false)
            setFetching(true)
            // Check if user exists in DontManage
            call.get('dontmanage.client.get_value', {
                doctype: 'User',
                filters: [['email', '=', email]],
                fieldname: 'name'
            }).then((user) => {
    ...
    return <form onSubmit={handleSubmit(onSubmit)}>
        <Stack gap='2' pt='2'>
                ...
                <Stack>
                    <Box>
                        ...
                        <TextField.Root
                            id='first_name'
                            {...register('first_name', {
                                // Not required if user exists in DontManage
            ...
            <Text size={'2'} color='gray'>
                {userExists ? 'This user already exists in DontManage. Add them to Raven?' : 'An invite will be sent on their email.'}
            </Text>
