...
const PushNotificationToggle = (props: Props) => {
    ...
    // @ts-expect-error
    const [pushNotificationsEnabled, setPushNotificationsEnabled] = useState(window.dontmanagePushNotification?.isNotificationEnabled())
    ...
    // @ts-expect-error
    if (!window.dontmanage?.boot.push_relay_server_url && !isPushAvailable) {
        return null
    }
    return (
