...
import { useDontManageCreateDoc, useDontManageFileUpload, useDontManageUpdateDoc, useSWRConfig } from 'dontmanage-react-sdk'
...
const AddWorkspaceForm = ({ onClose }: { onClose: (workspaceID?: string) => void }) => {
    ...
    const { createDoc, loading: creatingDoc, error } = useDontManageCreateDoc<RavenWorkspace>()
    const { updateDoc, loading: updatingDoc } = useDontManageUpdateDoc()

    const { upload, loading: uploadingFile, error: fileError } = useDontManageFileUpload()
