...
const ViewFilesContent = () => {
    ...
    const { data: count, error: countError } = useDontManageGetCall<{ message: number }>("raven.api.raven_message.get_count_for_pagination_of_files", {
    ...
    const { data, error, isLoading } = useDontManageGetCall<{ message: FileInChannel[] }>("raven.api.raven_message.get_all_files_shared_in_channel", {
        "channel_id": channelID,
        "file_name": debouncedText,
        "file_type": fileType === 'any' ? undefined : fileType,
