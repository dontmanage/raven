...
const useChatStream = (channelID: string, scrollRef: MutableRefObject<HTMLDivElement | null>) => {
    ...
    const { call: fetchOlderMessages, loading: loadingOlderMessages } = useDontManagePostCall('raven.api.chat_stream.get_older_messages')
    const { call: fetchNewerMessages, loading: loadingNewerMessages } = useDontManagePostCall('raven.api.chat_stream.get_newer_messages')
    ...
    const { data, isLoading, error, mutate } = useDontManageGetCall<GetMessagesResponse>('raven.api.chat_stream.get_messages', {
    ...
    const { call: trackVisit } = useDontManagePostCall('raven.api.raven_channel_member.track_visit')
    /**
     ...
     * This is maintained by the useSWR hook (useDontManageGetCall) here.
     *
    ...
    useDontManageDocumentEventListener('Raven Channel', channelID ?? '', () => { })

    // If there are new messages in the channel, update the messages
    useDontManageEventListener('message_created', (event) => {
    ...
    useDontManageEventListener('message_edited', (event) => {
    ...
    useDontManageEventListener('message_deleted', (event) => {
    ...
    useDontManageEventListener('message_reacted', (event) => {
        mutate(d => {
            if (event.message_id && d) {
                const newMessages = d.message.messages.map((message) => {
                    if (message.name === event.message_id) {
    ...
    useDontManageEventListener('message_saved', (event) => {
