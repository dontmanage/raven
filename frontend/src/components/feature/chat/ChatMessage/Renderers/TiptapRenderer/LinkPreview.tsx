...
const LinkPreview = memo(({ messageID }: { messageID: string }) => {
    ...
    const { data } = useDontManageGetCall<{ message: LinkPreviewDetails[] }>('raven.api.preview_links.get_preview_link', {
    ...
    const { call } = useDontManagePostCall('raven.api.preview_links.hide_link_preview')
