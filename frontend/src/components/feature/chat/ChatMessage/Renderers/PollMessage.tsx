...
import { useDontManageDocumentEventListener, useDontManageGetCall, useDontManagePostCall } from "dontmanage-react-sdk"
...
export const PollMessageBlock = ({ message, user, ...props }: PollMessageBlockProps) => {
    ...
    const { data, error, mutate } = useDontManageGetCall<{ message: Poll }>('raven.api.raven_poll.get_poll', {
    ...
    useDontManageDocumentEventListener('Raven Poll', message.poll_id, () => {
        mutate()
    })
...
const SingleChoicePoll = ({ data, messageID }: { data: Poll, messageID: string }) => {
    ...
    const { call } = useDontManagePostCall('raven.api.raven_poll.add_vote')
