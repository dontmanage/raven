...
const WorkflowTransitionOptions = ({ doctype, docname, mutate }: { doctype: string, docname: string, workflow: any, mutate: VoidFunction }) => {
    ...
    const { data: transitions, mutate: refreshTransitions } = useDontManageGetCall<{ message: WorkFlowButtonData[] }>('dontmanage.model.workflow.get_transitions', {
        'doc': {
            'doctype': doctype,
            'name': docname
        }
    ...
    const applyTransition = (action: string) => {
        call.post('dontmanage.model.workflow.apply_workflow', {
            'doc': {
                'doctype': doctype,
                'name': docname
            },
