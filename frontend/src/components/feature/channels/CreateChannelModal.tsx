...
const CreateChannelContent = ({ updateChannelList, isOpen, setIsOpen }: { updateChannelList: VoidFunction, setIsOpen: (v: boolean) => void, isOpen: boolean }) => {
    ...
    const { data: isAdmin } = useDontManageGetCall<{ message: boolean }>('raven.api.workspaces.is_workspace_admin', { workspace: workspaceID }, workspaceID ? undefined : null)
    ...
    const { createDoc, error: channelCreationError, loading: creatingChannel, reset: resetCreateHook } = useDontManageCreateDoc()
