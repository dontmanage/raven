...
export const UpdateAdminStatusButton = ({ user, channelID, updateMembers }: UpdateAdminStatusButtonProps) => {
    ...
    const { updateDoc, loading: updatingMember, reset } = useDontManageUpdateDoc()

    const { data: member } = useDontManageGetCall<{ message: { name: string } }>('dontmanage.client.get_value', {
