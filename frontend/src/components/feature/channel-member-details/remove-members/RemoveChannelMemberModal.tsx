import { useDontManageDeleteDoc, useDontManageGetCall, useSWRConfig } from 'dontmanage-react-sdk'
...
export const RemoveChannelMemberModal = ({ onClose, member }: RemoveChannelMemberModalProps) => {
    ...
    const { deleteDoc, error, loading: deletingDoc } = useDontManageDeleteDoc()
    ...
    const { data: memberInfo, error: errorFetchingChannelMember } = useDontManageGetCall<{ message: { name: string } }>('dontmanage.client.get_value', {
