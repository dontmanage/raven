    ...
    return <Stack gap='4'>
        ...
        <Stack>
            ...
            <HelperText size='2'>
                ...
                You can also use functions like <Code>{`{{ dontmanage.utils.today() }}`}</Code>.
                <br />
                For a complete list of functions, see <Link href='https://docs.dontmanage.io/framework/user/en/api/jinja'>DontManage's Jinja documentation</Link>.
            </HelperText>
        </Stack>
...
const RecipientsTab = () => {
    ...
    return <Stack gap='4'>
        ...
        <Text size='2'>
            ...
            You can use the following variable to get the recipient: <Code color='gray' size='2'>{`{{ dontmanage.db.get_value('Employee', doc.employee_id, 'user') }}`}</Code>
        </Text>
    </Stack>
}
    ...
    return <Controller
        ...
        render={({ field, fieldState }) => (
            <TextField.Root
                ...
                onBlur={field.onBlur}
                placeholder='e.g. {{ dontmanage.db.get_value("Employee", doc.employee_id, "user_id") }}'
                aria-invalid={fieldState.error ? 'true' : 'false'}
            />
        )}
    />
