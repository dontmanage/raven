...
import { useDontManageGetDocCount, useDontManagePostCall, useSWRConfig } from 'dontmanage-react-sdk'
...
const WorkspaceMemberCount = ({ workspace }: { workspace: string }) => {
    const { data } = useDontManageGetDocCount('Raven Workspace Member', [['workspace', '=', workspace]], true)
...
const OtherWorkspaceItem = ({ workspace }: { workspace: WorkspaceFields }) => {
    ...
    const { call } = useDontManagePostCall('raven.api.workspaces.join_workspace')
