 ...
 * @param doctype Name of the DontManage Doctype for which total count will be returned.
...
export const usePaginationWithDoctype = (doctype: string, pageLength: number, filters?: Filter[]): DoctypePaginationHookReturnType => {
    ...
    const { data: count, error, mutate } = useDontManageGetDocCount(doctype, filters)
