...
class DontManagePushNotification {
    static get relayServerBaseURL() {
        return window.dontmanage?.boot.push_relay_server_url
    }
    ...
    async fetchWebConfig() {
        ...
        try {
            let url = `${DontManagePushNotification.relayServerBaseURL}/api/method/notification_relay.api.get_config?project_name=${this.projectName}`
    ...
    async registerTokenHandler(token) {
        try {
            let response = await fetch(
                "/api/method/dontmanage.push_notification.subscribe?fcm_token=" +
                token +
                "&project_name=" +
                this.projectName,
                {
    ...
    async unregisterTokenHandler(token) {
        try {
            let response = await fetch(
                "/api/method/dontmanage.push_notification.unsubscribe?fcm_token=" +
                token +
                "&project_name=" +
                this.projectName,
                {
...
export default DontManagePushNotification